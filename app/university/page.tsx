...
import { BookOpen, MapPin, CheckCircle, Clock, ChevronDown, ChevronRight, ChevronUp, ChevronLeft, Play, Info, BarChart, GraduationCap, LogOut, Plus } from 'lucide-react';
...
export default function UniversityPortalPage() {
    ...
    const checkEnrollment = async (userId: string) => {
        ...
        const { data: allUniversities } = await supabase.from('universities').select('*').eq('is_active', true);
        ...
        // If no enrollments, show registration by default
                ...
                if (row.topic) {
                    ...
                    if (row.subtopic && row.subtopic.topic_id === row.topic.id && topic) {
                        if (topic.subtopics && !topic.subtopics.find(st => st.id === row.subtopic.id)) {
                            topic.subtopics.push(row.subtopic);
                        }
                    }
                }
            });
                    ...
                    {showRegistration ? (
                        ...
                        <div className="max-w-2xl mx-auto">
                            ...
                            <div className="bg-white p-8 rounded-[2rem] shadow-xl border border-gray-100">
                                <p className="text-slate-500 mb-6 text-sm font-medium">Search for your institution to gain access to curriculum-aligned learning paths and practice sessions.</p>
                                            ...
                                            <div className="p-8 space-y-4">
                                                {subjectItem.topics.length > 0 ? subjectItem.topics.map(topic => (
                                                    <div key={topic.id} className="border border-gray-100 rounded-2xl overflow-hidden bg-white shadow-sm transition-all hover:border-indigo-100">
                                                        ...
                                                        >
                                                            <div className="flex items-center gap-4">
                                                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold transition-all ${expandedTopic === topic.id ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-slate-500'}`}>
                                                                    {topic.subtopics.length}
                                                                </div>
                                                                <div>
                                                                    <h4 className="font-bold text-slate-800">{topic.name}</h4>
                                                                    <p className="text-xs text-slate-500 mt-0.5">Click to view lessons</p>
                                                                </div>
                                                            </div>
                                                                    ...
                                                                    return (
                                                                        ...
                                                                        >
                                                                            <div className="flex items-center gap-4">
                                                                                ...
                                                                                <span className={`text-[13px] font-black tracking-tight ${status.isRead || status.isMastered ? 'text-slate-900' : 'text-slate-500'}`}>
                                                                                    {subtopic.name}
                                                                                </span>
                                                                            </div>
                                                                            <div className="flex items-center gap-3">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden group">
                                            ...
                                            <div className="relative z-10">
                                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-4">Total Sessions</span>
                                                <h3 className="text-4xl font-black text-slate-900">{stats?.totalSessions || 0}</h3>
                                                <p className="text-xs text-slate-500 mt-2 font-bold uppercase tracking-tighter">Practice sessions completed</p>
                                            </div>
                                        </div>
                                        <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden group">
                                    ...
                                    <div className="bg-slate-900 p-10 rounded-[3rem] shadow-xl relative overflow-hidden">
                                        ...
                                        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                                            <div className="max-w-md text-center md:text-left">
                                                ...
                                                <p className="text-indigo-200/70 text-sm leading-relaxed mb-6 font-medium">Continue your journey through the curriculum. Every session improves your mastery score and unlocks new institutional milestones.</p>
                                                <button
                                                    onClick={() => setActiveTab('library')}
